...
<img src="assets/继承.png" alt="继承" style="zoom:50%;" />
...
- 本质：函数调用关系在运行时 “动态选择”，依赖对象的真实类型。
- 关键：必须满足两个条件 ——① 调用的是**虚函数**（基类中用 `virtual` 声明，派生类重写）；② 通过**基类的指针或引用**调用。

**动态绑定的适用场景：**

​	**仅有一种场景**：通过**基类的指针或引用**，调用被派生类**重写**的**虚函数**。这是 C++ 实现 “多态” 的核心机制 —— 同一调用语句，根据指向的对象类型不同，执行不同的函数。
...
| 依赖条件           | 无需虚函数，不依赖多态                 | 必须满足：① 虚函数重写；② 基类指针 / 引用 |
